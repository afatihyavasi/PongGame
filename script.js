...
const render = ()=>{ 
    ...
    //Draw paddles
    drawRect(user.x+=user.dx, user.y+=user.dy,user.width,user.height,user.color);
...
const runBall = ()=>{
    ...
    if(checkCollision(ball,side)){
       //Topun cubuga carpma acisina gore topun yonu ve hizi belirleniyor
        ...
        if(getComputedStyle(canvas).visibility == 'visible' & getComputedStyle(gameOverScreen).visibility == 'hidden'){
            smashSong.play();
        }
...
const checkWallforPlayers = (side) => {
    ...
    //Bottom wall
    if(side.y + side.height > canvas.height)side.y = canvas.height-side.height;
}
...
const radioSelection = ()=>{
    ...
    if(selectAgain[i].checked)
    selected = selectAgain[i].value;
    }
}
...
const gameOverScreenCreate = ()=>{
    ...
    if(user.score == 5 || com.score == 5)gameOverScreen.style.visibility = 'visible'; 
}
